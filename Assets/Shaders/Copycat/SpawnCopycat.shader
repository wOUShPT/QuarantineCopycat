...
Shader "SpawnCopycat"
{
	Properties
	{
		...
		[ASEEnd]_Dissolve("Dissolve", Range( -10 , 0.9999)) = 0.01
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float4 temp_cast_1 = (( 1.0 - _Dissolve )).xxxx;
				
				surfaceDescription.Color = _BaseColor.rgb;
				surfaceDescription.Emission = 0;
				surfaceDescription.Alpha = pow( saturate( tex2D( _NoiseTexture, texCoord30 ) ) , temp_cast_1 ).r;
		...
		Pass
		{
					...
					)
			{
				...
				surfaceDescription.Alpha = pow( saturate( tex2D( _NoiseTexture, texCoord30 ) ) , temp_cast_0 ).r;
				surfaceDescription.AlphaClipThreshold = _AlphaCutoff;
...
ASEEND*/
//CHKSM=20D5631A50095018682904B54D88E96B1E450123